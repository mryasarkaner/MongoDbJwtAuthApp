...
using MinimalApiMongoAuthProject.Dtos;
...
namespace MinimalApiMongoAuthProject.Controllers
{
    ...
    public class AuthenticationController : ControllerBase
    {
        ...
        private async Task<RegisterResponse> RegisterAsync(RegisterRequest request)
        {
            ...
            try
            {
                ...
                if(userExists != null) return new RegisterResponse { Message = "User already exist", Success = false };
                ...
                //user is created, then add  user to role


                var addUserToRoleResult = await _userManager.AddToRoleAsync(userExists, "USER");
                if (!addUserToRoleResult.Succeeded) 
                    return new RegisterResponse { Message = $"Create user succeeded but could not add user to role" +
                        $" {addUserToRoleResult?.Errors?.First()?.Description}", Success = false }; //all is still well..
        ...
        private async Task<LoginResponse> LoginAsync(LoginRequest request)
        {
                ...
                var claims = new List<Claim>
            {
                ...
                var key = new SymmetricSecurityKey(Encoding.UTF8.GetBytes("1swek3u4uo2u4a6e1swek3u4uo2u4a6e1swek3u4uo2u4a6e"));
